...
export default function AutoExitLogsPage() {
  ...
  useEffect(() => {
    async function fetchLogs() {
      ...
      setError(null);
      try {
        const res = await fetch("/api/activity-log?type=auto_exit", { cache: "no-store" });
  ...
  return (
    <div className="p-6  mx-auto">
      ...
      {!loading && logs.length === 0 && <div>No auto exit logs found.</div>}
              ...
              <tbody>
                {logs.map((log, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="px-4 py-2 font-semibold">{log.reason || log.action || "-"}</td>
                    <td className="px-4 py-2 break-all">{log.details || log.message || "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
