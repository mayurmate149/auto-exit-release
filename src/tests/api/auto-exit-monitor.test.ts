...
jest.mock('../../lib/mongodb', () => ({
  connectToDatabase: jest.fn(() => ({ db: { collection: jest.fn(() => ({ updateOne: jest.fn(), insertOne: jest.fn() })) } }))
}));
...
describe('auto-exit-monitor API', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });

  it('POST start: should start monitoring', async () => {
  ...
  it('POST stop: should stop monitoring', async () => {
  ...
  it('GET: should return state', async () => {
    const res = await GET();
    const data = await res.json();
    // @ts-ignore
